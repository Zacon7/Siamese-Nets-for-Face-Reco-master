...
def test_pairwise_distances():
    ...
    for squared in [True, False]:
        ...
        res_tf = _pairwise_distances(embeddings, squared=squared)
        assert np.allclose(res_np, res_tf.numpy(),1e-2)
...
def test_pairwise_distances_are_positive():
    ...
    for squared in [True, False]:
        res_tf = _pairwise_distances(embeddings, squared=squared)
        assert np.all((res_tf >= 0.0).numpy())
...
def test_gradients_pairwise_distances():
    ...
    for squared in [True, False]:
        ...
        grads = torch.autograd.grad(dists, embeddings, grad_outputs=torch.ones_like(dists))
        assert not np.any(torch.isnan(grads[0]).numpy()), "Gradient shouldn't be nan, squared={}".format(squared)
...
def test_anchor_positive_triplet_mask():
    ...
    assert np.allclose(mask_np, mask_tf_val.numpy())
...
def test_simple_batch_all_triplet_loss():
    ...
    for squared in [True, False]:
        ...
        loss_tf_val, fraction_val = (loss_tf, fraction)
